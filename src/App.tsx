    ...
    <div className="bg-zinc-900/50 border border-zinc-800 rounded-2xl p-6 mb-8">
      <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
        <h3 className="text-lg font-medium text-emerald-400 uppercase tracking-widest flex items-center gap-2">
          <Activity className="w-4 h-4" /> {type} Playground
        </h3>
        <div className="flex items-center gap-2">
          <form onSubmit={handleSubmit} className="flex gap-2">
            ...
            <button 
              type="submit"
              className="bg-emerald-600 hover:bg-emerald-500 text-white p-2 rounded-lg transition-all active:scale-95"
            >
      ...
      <div className="min-h-[200px] flex items-center justify-center border-2 border-dashed border-zinc-800 rounded-xl p-8 overflow-x-auto">
        <div className={`flex gap-4 ${type === 'Stack' ? 'flex-col-reverse' : 'flex-row'}`}>
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-zinc-100 font-sans selection:bg-emerald-500/30">
      ...
      <main className="max-w-6xl mx-auto p-6 pb-24">
        {/* Tab Navigation */}
        <div className="flex gap-4 p-1 bg-zinc-900 border border-zinc-800 rounded-2xl mb-12 w-fit">
          <button 
            onClick={() => setActiveTab('linear')}
            className={`px-6 py-2 rounded-xl transition-all font-medium flex items-center gap-2 ${activeTab === 'linear' ? 'bg-emerald-600 text-white shadow-lg' : 'text-zinc-400 hover:text-white'}`}
          >
            <Layers className="w-4 h-4" /> Linear
          </button>
          <button 
            onClick={() => setActiveTab('nonlinear')}
            className={`px-6 py-2 rounded-xl transition-all font-medium flex items-center gap-2 ${activeTab === 'nonlinear' ? 'bg-emerald-600 text-white shadow-lg' : 'text-zinc-400 hover:text-white'}`}
          >
          ...
          {activeTab === 'linear' ? (
            ...
            >
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-12">
                <div className="bg-zinc-900 border border-zinc-800 p-6 rounded-2xl">
                  <h4 className="text-emerald-400 font-bold mb-2">Array (อาร์เรย์)</h4>
                  <p className="text-sm text-zinc-400">เก็บข้อมูลชนิดเดียวกันไว้ในหน่วยความจำที่เรียงต่อกัน เข้าถึงได้รวดเร็วผ่าน Index</p>
                </div>
                <div className="bg-zinc-900 border border-zinc-800 p-6 rounded-2xl">
                  <h4 className="text-emerald-400 font-bold mb-2">Stack (สแต็ก)</h4>
                  <p className="text-sm text-zinc-400">โครงสร้างแบบ LIFO (Last-In First-Out) ข้อมูลที่เข้าทีหลังจะถูกนำออกก่อนเสมอ</p>
                </div>
                <div className="bg-zinc-900 border border-zinc-800 p-6 rounded-2xl">
                  <h4 className="text-emerald-400 font-bold mb-2">Queue (คิว)</h4>
                  <p className="text-sm text-zinc-400">โครงสร้างแบบ FIFO (First-In First-Out) ข้อมูลที่เข้ามาก่อนจะถูกนำออกก่อน</p>
                </div>
              </div>
            ...
            >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="bg-zinc-900 border border-zinc-800 p-8 rounded-3xl group hover:border-emerald-500/50 transition-all">
                  ...
                  <div className="relative h-48 flex items-center justify-center bg-zinc-950 rounded-2xl border border-zinc-800 overflow-hidden">
                    ...
                    <div className="flex flex-col items-center gap-8">
                      ...
                      <div className="flex gap-12">
                        <div className="w-10 h-10 rounded-full border-2 border-emerald-500/50 flex items-center justify-center text-zinc-400">A</div>
                        <div className="w-10 h-10 rounded-full border-2 border-emerald-500/50 flex items-center justify-center text-zinc-400">B</div>
                      </div>
                    </div>
                ...
                <div className="bg-zinc-900 border border-zinc-800 p-8 rounded-3xl group hover:border-emerald-500/50 transition-all">
                  ...
                  <div className="relative h-48 flex items-center justify-center bg-zinc-950 rounded-2xl border border-zinc-800 overflow-hidden">
                    ...
                    <div className="relative w-32 h-32">
                      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-8 h-8 rounded-full bg-emerald-500/20 border border-emerald-500 flex items-center justify-center text-xs">1</div>
                      <div className="absolute bottom-0 left-0 w-8 h-8 rounded-full bg-emerald-500/20 border border-emerald-500 flex items-center justify-center text-xs">2</div>
                      <div className="absolute bottom-0 right-0 w-8 h-8 rounded-full bg-emerald-500/20 border border-emerald-500 flex items-center justify-center text-xs">3</div>
      ...
      <footer className="fixed bottom-0 left-0 right-0 bg-zinc-900/80 backdrop-blur-xl border-t border-zinc-800 p-4 z-50">
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-2 text-zinc-500 text-sm">
            ...
            <span>ข้อมูลจะถูกบันทึกอัตโนมัติไปยัง Database</span>
          </div>
          <button 
            onClick={globalReset}
            className="flex items-center gap-2 px-4 py-2 bg-red-500/10 hover:bg-red-500 text-red-500 hover:text-white rounded-xl transition-all font-medium text-sm"
          >
